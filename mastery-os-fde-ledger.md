...
## 10. The Grader Was Marking Work It Had Never Been Shown

### 📖 The Plain-English Story
The previous entry ended with one scorecard out of seventeen failing, and an honest note saying we did not yet know whether the fault was the scorecard or the test we had written for it. Resolving that is this entry.

It was neither. It was the grader.

The failing scorecard was the one for marketing copy. Two of its four questions ask whether the writing did what it was *told* to do — did it stick to the assigned angle, did it respect the word limit and the banned words. Perfectly sensible questions. But the grader was never given the assignment. It received the finished post and the scorecard, and nothing about what had been asked for. It was being asked to check homework against instructions it could not see.

The evidence was unmistakable once we looked at the individual questions instead of the total. On "were the stated constraints met exactly," across four test outputs deliberately built to be terrible, poor, decent, and excellent, the grader returned 2, 2, 2, 1. The output that broke every single rule scored *higher* than the one that followed every rule perfectly — the good one had honestly flagged a tension in its own instructions, and the grader read that candour as an admission of failure. That is not a noisy signal. There is no signal. The question was unanswerable as posed.

It also explained why sixteen scorecards passed and one failed. The other sixteen ask things you can check by reading the output alone: does every claim carry a link, does every risky action state how to undo it. Only the marketing one asks a *relational* question — does this match what it was asked for — and that is the only kind of question a blindfolded grader cannot answer.

The fix was obvious: show the grader the assignment. And that is where it got interesting, because the obvious fix was a trap.

We had verified all seventeen scorecards against the grader as it existed. Changing what the grader sees changes the grader. Every one of those seventeen verifications would have quietly become a statement about a machine that no longer existed — and every one would have kept displaying "verified," because we had bound the verification to the *scorecard's* version and never to the grader's. We had built a careful tripwire for one door and left the other one open.

So we closed it first, before making the change. The system now takes a fingerprint of exactly what the grader is shown and stores it alongside each verification. Then we made the change. All seventeen verifications immediately flipped to "the grader changed — re-run me," each one explicitly saying the scorecard itself was fine. Nothing had to be remembered by a person.

Re-run, the marketing scorecard works. The grader now writes things like *"the post uses the explicitly forbidden word 'journey'"* — a sentence it was physically incapable of producing the day before, because it had never seen the list.

### 🛠️ The Hard Engineering Facts
* **The Vulnerability:** `quality.build_prompt` passed the grader the success criteria, the return, and the rubric — and deliberately withheld the context payload, so the grader could not re-do the task. Correct. But it also withheld `brief.objective` and `brief.constraints`, which was never a decision, just the shape the function happened to have. `content`'s rubric asks `executes_the_given_angle` (the angle lives in `objective`) and `declared_constraints_honored` (the word cap lives in `constraints`). Two of four dimensions were unanswerable by construction, and the aggregate averaged real signal with pure noise.
* **The "Aha" Discovery — read the dimensions, not the total:** `content` failed as `[1.5, 1.25, 2.75, 2.5]` against intended `[0, 1, 2, 3]`. The total says "noisy." The per-dimension breakdown says something far more specific: `declared_constraints_honored` scored **2, 2, 2, 1** — monotonically *backwards*. A metric that is merely noisy and a metric that is inverted look identical in aggregate and have completely different causes. The diagnosis was only available one level down.
* **Confirmed Against The Artifact, Not Inferred:** before changing anything we rendered the actual grading prompt for `content`'s top rung and string-searched it. The 150-word limit: absent. The angle: absent. After the fix, both present, and the context payload still absent. The claim was made checkable before it was made.
* **The Second Trap, Closed Before It Fired:** calibration was keyed `(agent, rubric_version)`. Changing the grading prompt changes the instrument for **all seventeen roles at once** while touching no rubric — so every calibration would have gone on reporting `calibrated` about a grader that no longer existed. This is the *same* trap entry 9 describes closing for rubrics, arriving through the door version-binding does not cover. Found by asking "what else is this attesting to that isn't in the key," before spending anything.
* **The FDE Fix — fingerprint what the grader is shown:** `quality.prompt_fingerprint()` renders the prompt against fixed sentinel fixtures and hashes it; the key becomes `agent|rubric_version|prompt_fingerprint`. Keyed, not compared-after-lookup — a lookup that found the entry and *then* checked a field leaves a path where the check can be forgotten. There is no such path if the key is simply wrong.
* **Two Design Decisions That Keep It Believable:** it hashes the *rendered prompt*, not the module's source — a source hash would void seventeen ladders over a docstring edit, and the obvious way to stop that noise is to stop believing the fingerprint. And the rubric is passed *into* the renderer rather than looked up inside it, so the hash is independent of the rubrics file; the two axes move separately instead of each dragging the other. Pinned by `test_the_fingerprint_does_not_move_when_a_rubric_moves`.
* **A Distinct Verdict, Not A Reused One:** `PROMPT_CHANGED` is separate from `STALE` because the repairs differ — stale means *rewrite the rungs against new anchors*, prompt-changed means *the rungs are still correct, just re-run them*. Collapsing them would send the next reader to rewrite test cases that were never wrong.
* **Backfill That Was Earned, Not Asserted:** the seventeen existing records were rekeyed onto the old fingerprint rather than re-run — defensible only because it is checkable, so we checked it. `mastery/quality.py` was last committed in `98a47b9`, predating both calibration commits; and the refactor that split the renderer out was verified to leave the rendered prompt **byte-identical for all 17 agents** before the rekey was applied. Recorded in `evals/calibration.json` under `_fingerprint_backfill`.
* **The Result:** `content` went `[1.5, 1.25, 2.75, 2.5]` (spread 1.5, **not monotonic**) → `[0.25, 1.25, 2.75, 3.0]` (spread 2.75, monotonic). **17 of 17 calibrated.** The open question from entry 9 — rubric or ladder — is answered: neither.
* **The Control We Could Actually Run:** the other sixteen rubrics did not change, so their movement measures the prompt change plus ordinary variance. Across 64 rung values: mean signed delta **−0.038**, mean absolute **0.125**, **38 of 64 rungs identical**, `fact-checker` reproduced byte-for-byte, every verdict held, monotonicity 16 → 17. Against entry 9's own recorded finding that `fact-checker`'s middle rung moved `1.2 → 1.6` between two runs of the *same* prompt, a 0.125 mean shift is at or below the noise floor. The fix did what the diagnosis predicted and nothing else.
* **The Trap We Refused:** the cheap fix was to restate the angle and the word limit inside `success_criteria`, which *does* reach the grader. Costs nothing, makes `content` pass immediately, and leaves every real marketing run still graded blind on half its dimensions — while the calibration now reads `calibrated`. It would have been strictly harder to find the second time. Written into the backlog as a rejected option with its reasoning, because a rejected option that is not recorded gets re-proposed.
* **A Latent Bug The Fix Exposed, Caught Pre-Spend:** all 17 ladder briefs carried `constraints` and `out_of_scope` as JSON **lists**, while `TaskBrief` declares strings and every real brief is a string. Invisible for as long as the grader could not see those fields — the moment it could, all 17 ladders would have rendered a Python list repr that no production run ever produces, calibrating the instrument against a prompt that does not exist. The probe had been assembling briefs with `build()`, which checks only the task id. It now calls `brief.validate()`, which is what would have caught it originally. Cost of finding it this way: nothing. Cost of finding it after the re-run: 68 calls and a set of numbers that looked fine.
* **A Condition That Was About To Resolve Itself Backwards:** `evals/baselines.json` said to treat its baselines as provisional *"until their rubric has a passing ladder."* Hours later all 17 had one — and that licenses none of them, because every baseline score was produced by the *old* grading prompt. A condition that reads as satisfied while the risk it guards against is still live is worse than no condition at all. Rewritten per baseline. `content`'s is now named for what it is: n=1, scored 2.75/3 by an instrument since proven unable to separate a deliberately awful output from a good one. Not weak evidence — not evidence.
* **What It Cost:** **$4.093453** across 68 priced grader calls; mean $0.0601978 per call, $0.240791 per ladder. That is **+12.7% per call** over the previous generation, from the larger prompt. The projection made before the run was ~$4.48 and came in **8.6% low** — recorded so the next estimate starts from a rate that includes the new blocks rather than repeating the error.
* **An Unplanned Property Worth Keeping:** because the fingerprint is part of the key, re-running wrote seventeen *new* records beside the seventeen old ones — `evals/calibration.json` now holds 34. The file is no longer a snapshot of current state; it is the before/after of a measured instrument change. Kept deliberately and documented under `_generations` rather than pruned.

---

## 11. The Role Doc Said No, and the Scorecard Would Have Said Zero

### 📖 The Plain-English Story

The operator wanted the FaithFeed app to look high-end, and wanted the work run through
the system rather than done by hand. The obvious blocker was that no agent owned visual
design: the `ui-ux` role doc forbids it in as many words — *"Structural description only —
no visual mockups, no CSS, no implementation"* — and `mobile-dev` "executes against a spec,
does not author one." So "make it look high-end" routed to nobody.

The obvious fix was equally clear: `ui-ux` is the design agent, so widen its doc to cover
colour and type. One file, four lines. The draft plan recommended exactly that, and called
it the smallest change available.

It is the largest one. The agent doc is not what grades the work. `ui-ux`'s scorecard
carries a dimension called `structural_only`, and its bottom anchor — the zero — is *"CSS,
mockups, or implementation detail supplied."* Its top anchor is *"visual and implementation
choices left open."* Widen the doc without touching the scorecard and every visual-design
task scores zero on that dimension **by construction**, forever, while the manager keeps
accepting the work as correct. The agent would be doing exactly what it was told and being
marked down for it, and the trend would show a competent agent slowly getting worse.

Touch the scorecard to fix that, and the version bumps, and the version bump voids the
passing calibration that says `ui-ux`'s scorecard can tell good work from bad. The cheap
change costs a re-run of a metered ladder. The change that looked expensive — a new
`visual-design` agent, five files — voids nothing at all.

### 🛠️ The Hard Engineering Facts
* **The Vulnerability:** two documents describe one role and only one of them is read at grading time. `docs/agents/ui-ux.md` defines what the agent should do; `docs/agents/quality_rubrics.md` defines what it is scored on. `quality.build_prompt` passes the rubric and never the agent doc — correctly, since a grader holding the role definition would start marking against the doc instead of the anchors. The consequence is that editing a role's doc and editing its rubric are not interchangeable, and the system has no check that they still agree.
* **The "Aha" Discovery — the doc is a prohibition, the rubric is an incentive:** they were saying the same thing here, which is why widening one looked sufficient. `ui-ux.md:33` forbids visual output and `structural_only` penalises it. Removing the prohibition leaves the penalty in place and pointed at the new job. The failure is silent in the worst way available: `status` stays `complete`, the manager verdict stays `accept`, and the only signal is a dimension that never rises above zero — which reads as an agent that cannot do its job rather than a scorecard measuring the wrong one.
* **Checked Before It Was Claimed:** `evals/calibration.json` holds `ui-ux|1|93074072ad14`, `discriminates: true`, spread 3.0, intended `[0,1,2,3]` → observed `[0,1.2,2.2,3]`. That entry is keyed on the rubric version, so any edit to `structural_only` invalidates it and the ladder rungs at the `ui-ux` key in `calibration_ladders.md` — written against the old anchor text — have to be rewritten before it can be re-run. The cost of the "small" fix is a rubric rewrite, a ladder rewrite, and a priced ladder run. The cost of the "large" fix is a new rubric at version 1, which voids nothing because nothing was ever bound to it.
* **The FDE Fix — a new role rather than a widened one:** `visual-design`, Tier 2, with its own doc, roster entry, rubric at version 1, and ladder. `ui-ux`'s rubric and calibration are untouched, verified by `git diff evals/calibration.json` returning empty and `quality_rubrics.md` showing 65 insertions and zero deletions. `probe_grader.py --status` reports 17 of 18 calibrated with every existing agent's line byte-identical — the new one reads `MISSING`, which is the honest state and not a regression.
* **The Uncalibrated Rubric Is Not A Gap Left Open By Accident:** `calibration.py` already draws this line — a missing calibration blocks *baselines*, never delegations and never scoring, because halting real work over unwritten paperwork would make an uncalibrated rubric worse than no rubric. So `visual-design` runs and scores from day one and simply cannot carry a baseline until its ladder is run. The ladder is written; running it is a priced call and is in the backlog rather than smuggled into this change.
* **A Second Silent Failure, Found Looking For The First:** every delegation resolves `Read`/`Glob`/`Grep` against `config.cwd`, which defaults to this repo. Every FaithFeed brief to date had therefore been reading the orchestrator's own source. What makes it worth recording is the failure *shape*: a wrong `cwd` does not raise. The tools succeed, return nothing, and the agent truthfully reports finding no files — at full price, in a form indistinguishable at the shell from a bad model. `mastery check` validated the roster, the schemas, and the caps, and never once looked at the one setting that decides which repository the work happens in. It does now.
* **The Workaround That Does Not Exist:** the reflex is to paste the files into the brief instead. `caps.max_context_bytes` is 60,000 and `brief.validate` *rejects* an over-limit payload rather than truncating it, so the app shell at 58,319 bytes cannot ship even alone once its role doc is attached. Worth stating because "just paste it" is the kind of workaround that gets tried once per person; the limit is a hard refusal, not a degradation.
* **A Measurement Bug Fixed By Deleting The Measurement:** the brief this work was meant to consume claimed the app did not compile — 87 errors from an in-progress `_archive/` move — against a repository with no `_archive/` directory and zero errors in `lib/`, and undercounted every repo-wide figure by 19–32% because whole feature directories were never walked. Both are invisible to a reader and obvious the moment the numbers are re-derived. The fix is not to correct the figures, which is how they broke: `scripts/ui_survey.py` generates them, so the document is rebuilt from output rather than edited in place. It computes the headline finding rather than asserting it — a font family named in code but absent from `pubspec.yaml` is reported as `referenced_but_not_declared`, which is a silent fallback and the reason the app renders in nothing like the face it names.
...
10. **"When you change the ruler, every past measurement is a claim about a ruler that no longer exists."** The sharpest judgment call in this ledger, and it is about *sequencing*. One of seventeen scorecards was failing; the diagnosis was that the grader was being asked whether the work followed instructions it had never been shown. The fix was one obvious line — show it the instructions. **The insight was recognising that the obvious fix would have silently invalidated the other sixteen verifications while every one of them kept displaying "verified."** We had bound each verification to the scorecard's version and never to the grader's, so a change that touched no scorecard would have voided them all undetectably. So we built the fingerprint *first*, landed it while it still verified as a no-op, and only then made the change — at which point all seventeen correctly flipped to "the grader moved, re-run me," each explicitly clearing the scorecard itself. Close with the honesty check: the sixteen unaffected scorecards moved a mean absolute 0.125 on a 3-point scale with 38 of 64 test points identical, which is below the run-to-run noise we had already documented — the change did what the diagnosis predicted and nothing more. And name the shortcut we refused: we could have restated the instructions inside a field the grader *does* see, which would have made the failing scorecard pass that night and left every real run still graded blind. It is in the backlog as a rejected option, with reasons, because an unrecorded rejection gets re-proposed.

11. **"Widening a job description does not widen the scorecard, and the scorecard is what pays."** The cleanest example in this ledger of a small change being the expensive one. An agent's role doc forbade visual work; the request needed visual work; the obvious fix was four lines in the doc. Explain what that misses: the doc governs the agent, but a *separate* file governs the grader, and the grader's rubric had a dimension whose zero-anchor was "supplied visual detail." Widen the doc alone and the agent does precisely what it is now told to do and is scored zero for it on every run — silently, because `status` stays `complete` and the manager keeps accepting the work. Land it on the sequencing: fixing the rubric to match bumps its version, and the version is part of the key on the calibration that certifies that rubric can tell good work from bad — so the four-line fix actually costs a rubric rewrite, a ladder rewrite, and a priced re-run, while standing up a whole new role costs nothing already proven. Finish with the check that made it a fact rather than an argument: `git diff evals/calibration.json` empty, `quality_rubrics.md` 65 insertions and 0 deletions, and every existing agent's calibration line byte-identical afterward.
